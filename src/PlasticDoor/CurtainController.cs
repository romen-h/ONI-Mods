		...
		public class Controller : GameStateMachine<Controller, Controller.Instance, Curtain>
		{
			...
			public override void InitializeStates(out BaseState default_state)
			{
				...
				closing
					...
					.Enter(smi => smi.master.flutterable.PlaySoundEffect("close", 2f))
					.OnAnimQueueComplete(closed);
				open
					.Enter(smi => smi.master.flutterable.Listening = false)
					.ParamTransition(isOpen, closing, IsFalse)
					.Enter(smi => smi.master.flutterable.PlaySoundEffect("open", 2f))
				...
				locked
					.Enter(smi => smi.master.flutterable.Listening = false)
					.Enter(smi => smi.master.flutterable.PlaySoundEffect("lock", 2f))
					.PlayAnim("lockedPre")
					.QueueAnim("locked")
					.ParamTransition(isLocked, unlocking, IsFalse);
				unlocking
					.PlayAnim("lockedPst")
					.Enter(smi => smi.master.flutterable.PlaySoundEffect("unlock", 1f))
					.OnAnimQueueComplete(closed);
			}
