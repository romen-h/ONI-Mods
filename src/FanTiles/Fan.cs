...
namespace Fans
{
	...
	public class Fan : KMonoBehaviour, ISim200ms, IGameObjectEffectDescriptor
	{
		...
		{
			if (overpressureMass < 0.0f) return new List<Descriptor>();
		...
		private void DoFan()
		{
			...
			if (Grid.Solid[outputCell]) return;
		...
		{
			if (primary_element.Mass <= 0) return false;

			CalculateDiseaseTransfer(exhaustPE, primary_element, 0.05f, out int disease_to_item1, out int disease_to_item2);
